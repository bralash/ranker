...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('weekNumber').addEventListener('change', handleWeekChange);
...
function processCSV(csv) {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        if (currentLine.length === headers.length) {
            ...
            const studentIndex = weeks[weekIndex].students.findIndex(s => s.name === studentName);
            const studentData = { name: studentName, quiz: parseInt(quizScore), lab: parseInt(labScore) };
...
function loadWeekData() {
    ...
    if (weekData) {
        students.forEach(student => {
            ...
            document.getElementById(`${student.name}-quiz`).value = weekStudent ? weekStudent.quiz : 0;
            document.getElementById(`${student.name}-lab`).value = weekStudent ? weekStudent.lab : 0;
        });
    } else {
        students.forEach(student => {
...
function updateWeeklyChart() {
    ...
    chartContainer.innerHTML = '<canvas></canvas><button class="export-btn" data-chart="weekly">Export Weekly Chart</button>';
    ...
    new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [
                {
                    label: 'Quiz',
                    data: students.map(student => parseInt(document.getElementById(`${student.name}-quiz`).value) || 0),
                    backgroundColor: 'rgba(75, 192, 192, 0.6)',
                },
                {
                    label: 'Lab',
                    data: students.map(student => parseInt(document.getElementById(`${student.name}-lab`).value) || 0),
...
function updateStudentTrendCharts() {
    ...
    students.forEach((student, index) => {
        ...
        const studentData = weeks.map(week => {
            const studentWeekData = week.students.find(s => s.name === student.name);
            return {
    ...
    document.querySelectorAll('.export-btn').forEach(button => {
        button.addEventListener('click', function() {
            ...
            if (chartId === 'weekly') {
                ...
                canvas = document.querySelectorAll('#studentTrendCharts canvas')[studentIndex];
            }
